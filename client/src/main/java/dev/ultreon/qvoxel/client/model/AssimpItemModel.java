...
public class AssimpItemModel implements ItemModel {
    private final Identifier resourceId;
    private BakedModel model;
    private List<Identifier> textures = new ArrayList<>();

    public AssimpItemModel(Identifier resourceId) {
    ...
    public void preload(QuantumClient client) {
        ...
        this.textures = textures.stream().map(s -> new Identifier(resourceId.location(), "items/" + s.split("\\.")[0])).toList();
    ...
    public void renderItem(GuiRenderer renderer, int x, int y) {
        ...
        modelMatrix.rotateX(org.joml.Math.toRadians(-25));
